...
class DemoCogsApp(App):
    ...
    def compose(self) -> ComposeResult:
        yield Vertical(
            Center(Button("Info MessageDialog", id="info-msg"),
    ...
    def on_info_msg(self, event: Button.Pressed) -> None:
        ...
        self.push_screen(MessageDialog("An informational message", title="Information", icon=icons.ICON_INFORMATION), self.msg_dialog_callback)

    @on(Button.Pressed, "#exclamation-msg")
    def on_exclamation_msg(self, event: Button.Pressed) -> None:
        self.push_screen(MessageDialog("DANGER!", title="Exclamation", icon=icons.ICON_EXCLAMATION), self.msg_dialog_callback)

    @on(Button.Pressed, "#question-msg")
    def on_question_msg(self, event: Button.Pressed) -> None:
        self.push_screen(MessageDialog("Do you want to save your work?", title="Question", icon=icons.ICON_QUESTION, flags=[labels.YES, labels.NO]), self.msg_dialog_callback)

    @on(Button.Pressed, "#warning-msg")
    def on_warning_msg(self, event: Button.Pressed) -> None:
        self.push_screen(MessageDialog("This is only a warning!", title="Warning", icon=icons.ICON_WARNING), self.msg_dialog_callback)

    @on(Button.Pressed, "#regular-msg")
    def on_regular_msg(self, event: Button.Pressed) -> None:
        self.push_screen(MessageDialog("Do you want to continue?", title="Regular", flags=[labels.OK, labels.CANCEL]), self.msg_dialog_callback)
